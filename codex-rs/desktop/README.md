...
| **PR-S2** | `AgentBridge::restart()` respawns the `codex-agent` child while keeping the existing event channel — GUI subscribers continue to receive `BridgeEvent`s without re-subscribing. The Reconnect button in the disconnect toast now actually restarts the agent. `agent_bridge` module ungated from the `gtk` feature so headless CI exercises an end-to-end restart integration test (spawn → submit → restart → submit again). New `AgentCommand` recipe struct lets tests inject `CODEX_DESKTOP_FORCE_ROLE=agent` into the spawned child. |
| **PR-W** | New `codex-drift-log` crate: append-only JSONL log of unknown server notifications + per-method counters with concurrent-writer-safe `Mutex<BufWriter<File>>` and a `replay()` helper for cross-session aggregation. `CodexBackend` now classifies notifications via `KnownVariantRegistry` (default registry covers the agent role's `agent/message_delta` + `agent/turn_completed`); anything outside the registry surfaces as `IncomingServerNotification::Unknown` and `AgentBridge` records each one to `~/.local/state/codex-desktop/drift-log.jsonl`. `AgentBridge::drift_log()` exposes the log to a future "Protocol Drift" diagnostic pane (architecture doc §3.3). |
| **PR-X** | New `codex-claude-backend` sibling crate: `ClaudeBackend` adapter implementing `AgentBackend` with `claude/*` wire methods (claude/initialize, claude/submit, claude/interrupt, claude/shutdown) + Claude-specific `KnownVariantRegistry` (claude/message_delta, claude/turn_completed, claude/content_block_delta). Mirrors `CodexBackend`'s NDJSON JSON-RPC actor architecture (writer task, pending map, broadcast fan-out) with `c{n}` request ids. Registered via `inventory::submit!` as `BackendId("claude-code")` so the desktop binary can list it in a backend picker. The default factory returns `BackendError::Closed` until a real `claude-code` host transport is plumbed in (out of scope for this scaffold) — the GUI is expected to fall back to the Codex backend when Claude is unselected or unconfigured. |

See [`../../docs/desktop-architecture.md`](../../docs/desktop-architecture.md) for the full architecture and the remaining roadmap (in-process Codex via codex-app-server-client, gettext, a11y CI gate, Flatpak vendoring).
